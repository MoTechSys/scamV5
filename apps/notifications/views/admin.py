...
class AdminNotificationDetailView(LoginRequiredMixin, AdminRequiredMixin, DetailView):
    """عرض تفاصيل إشعار."""
    model = Notification
    template_name = 'admin_panel/notifications/detail.html'
    context_object_name = 'notification'


class AdminNotificationEditView(LoginRequiredMixin, AdminRequiredMixin, UpdateView):
    """تعديل إشعار."""
    model = Notification
    form_class = NotificationForm
    template_name = 'admin_panel/notifications/edit.html'
    success_url = reverse_lazy('notifications:admin_list')
    
    def form_valid(self, form):
        messages.success(self.request, 'تم تحديث الإشعار بنجاح.')
        return super().form_valid(form)


class AdminNotificationDeleteView(LoginRequiredMixin, AdminRequiredMixin, View):
    """حذف إشعار."""
    ...
    def post(self, request, pk):
        ...
        notification.delete()
        messages.success(request, f'تم حذف الإشعار: {title}')
        return redirect('notifications:admin_list')
