...
class AdminCourseCreateView(LoginRequiredMixin, AdminRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        # ربط التخصصات بالمقرر
        majors = form.cleaned_data.get('majors', [])
        for major in majors:
            CourseMajor.objects.create(course=self.object, major=major)
        ...
        AuditLog.log(
            ...
            changes={'majors': [m.major_name for m in majors]},
...
class AdminCourseUpdateView(LoginRequiredMixin, AdminRequiredMixin, UpdateView):
    ...
    def get_initial(self):
        ...
        # تعبئة التخصصات المرتبطة حالياً
        initial['majors'] = self.object.course_majors.values_list('major_id', flat=True)
        return initial
    
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['majors'] = Major.objects.filter(is_active=True)
        context['assigned_majors'] = list(self.object.course_majors.values_list('major_id', flat=True))
        return context
    ...
    def form_valid(self, form):
        ...
        # حذف الربط القديم
        CourseMajor.objects.filter(course=self.object).delete()
